...
NS_LOG_COMPONENT_DEFINE("NrSlUeMacSchedulerDynamicMcs");
NS_OBJECT_ENSURE_REGISTERED(NrSlUeMacSchedulerDynamicMcs);

TypeId
NrSlUeMacSchedulerDynamicMcs::GetTypeId(void)
{
    static TypeId tid =
        TypeId("ns3::NrSlUeMacSchedulerDynamicMcs")
            .SetParent<NrSlUeMacScheduler>()
            .AddConstructor<NrSlUeMacSchedulerDynamicMcs>()
            .SetGroupName("nr")
            .AddAttribute("Mcs",
                          "The fixed value of the MCS used by this scheduler",
                          UintegerValue(14),
                          MakeUintegerAccessor(&NrSlUeMacSchedulerDynamicMcs::m_mcs),
            ...
            .AddAttribute("PriorityToSps",
                          ...
                          BooleanValue(true),
                          MakeBooleanAccessor(&NrSlUeMacSchedulerDynamicMcs::m_prioToSps),
                          MakeBooleanChecker())
            .AddAttribute("AllowMultipleDestinationsPerSlot",
                          "Allow scheduling of multiple destinations in same slot",
                          BooleanValue(false),
                          MakeBooleanAccessor(
                              &NrSlUeMacSchedulerDynamicMcs::m_allowMultipleDestinationsPerSlot),
                          MakeBooleanChecker());
    return tid;
}

NrSlUeMacSchedulerDynamicMcs::NrSlUeMacSchedulerDynamicMcs()
{
...
void
NrSlUeMacSchedulerDynamicMcs::DoCschedNrSlLcConfigReq(
...
NrSlUeMacSchedulerDynamicMcs::CreateDstInfo(
    const NrSlUeCmacSapProvider::SidelinkLogicalChannelInfo& params)
{
...
void
NrSlUeMacSchedulerDynamicMcs::DoRemoveNrSlLcConfigReq(uint8_t lcid, uint32_t dstL2Id)
{
...
void
NrSlUeMacSchedulerDynamicMcs::RemoveDstInfo(uint8_t lcid, uint32_t dstL2Id)
{
    NS_LOG_FUNCTION(this << lcid << dstL2Id);
...
NrSlLCGPtr
NrSlUeMacSchedulerDynamicMcs::CreateLCG(uint8_t lcGroup) const
{
...
NrSlLCPtr
NrSlUeMacSchedulerDynamicMcs::CreateLC(
...
void
NrSlUeMacSchedulerDynamicMcs::DoSchedNrSlRlcBufferReq(
...
uint8_t
NrSlUeMacSchedulerDynamicMcs::GetRandomReselectionCounter(Time rri) const
{
    uint8_t min;
    uint8_t max;
...
uint8_t
NrSlUeMacSchedulerDynamicMcs::GetLowerBoundReselCounter(uint16_t pRsrv) const
{
...
uint8_t
NrSlUeMacSchedulerDynamicMcs::GetUpperBoundReselCounter(uint16_t pRsrv) const
{
    NS_ASSERT_MSG(pRsrv < 100, "Resource reservation must be less than 100 ms");
...
void
NrSlUeMacSchedulerDynamicMcs::DoSchedNrSlTriggerReq(const SfnSf& sfn)
{
...
void
NrSlUeMacSchedulerDynamicMcs::DoNotifyNrSlRlcPduDequeue(uint32_t dstL2Id, uint8_t lcId, uint32_t size)
{
...
bool
NrSlUeMacSchedulerDynamicMcs::TxResourceReselectionCheck(const SfnSf& sfn,
                                                       uint32_t dstL2Id,
                                                       uint8_t lcId)
{
...
uint32_t
NrSlUeMacSchedulerDynamicMcs::LogicalChannelPrioritization(
    const SfnSf& sfn,
    std::map<uint32_t, std::vector<uint8_t>> dstsAndLcsToSched,
    AllocationInfo& allocationInfo,
    std::list<SlResourceInfo>& candResources)
...
void
NrSlUeMacSchedulerDynamicMcs::GetDstsAndLcsNeedingScheduling(
    const SfnSf& sfn,
    std::map<uint32_t, std::vector<uint8_t>>& dstsAndLcsToSched)
{
    NS_LOG_FUNCTION(this << sfn);
...
void
NrSlUeMacSchedulerDynamicMcs::AttemptGrantAllocation(const SfnSf& sfn,
                                                   uint32_t dstL2Id,
                                                   ...
                                                   const AllocationInfo& allocationInfo)
{
    ...
    const auto itDstInfo = m_dstMap.find(dstL2Id);
    // 打印 candResources
    // std::cout << "  Candidate Resources (" << candResources.size() << " resources available):" << std::endl;
    if(0)
    for (const auto& resource : candResources)
    {
        std::cout << "  Resource SFN: FrameNum: " << static_cast<int>(resource.sfn.GetFrame())
                  << ", SubFrameNum: " << static_cast<int>(resource.sfn.GetSubframe())
                  << ", SlotNum: " << static_cast<int>(resource.sfn.GetSlot()) << std::endl;

        // std::cout << "    Resource SFN: " << resource.sfn << std::endl;
        std::cout << "  PSCCH: numSlPscchRbs= " << resource.numSlPscchRbs
                  << ", slPscchSymStart= " << resource.slPscchSymStart
                  << ", slPscchSymLength= " << resource.slPscchSymLength << std::endl;

        std::cout << "  PSSCH: slPsschSymStart= " << resource.slPsschSymStart
                  << ", slPsschSymLength=" << resource.slPsschSymLength << std::endl;

        std::cout << "  SubChannel: slSubchannelStart= " << (int)resource.slSubchannelStart
                  << ", slSubchannelLength= " << (int)resource.slSubchannelLength
                  << ", slSubchannelSize= " << resource.slSubchannelSize << std::endl;

        std::cout << "  Pool Info: slPsfchPeriod= " << (int)resource.slPsfchPeriod
                  << ", slMinTimeGapPsfch= " << (int)resource.slMinTimeGapPsfch
                  << ", slMinTimeGapProcessing= " << (int)resource.slMinTimeGapProcessing
                  << ", slMaxNumPerReserve= " << resource.slMaxNumPerReserve << std::endl;
        std::cout << "==================" << "\n" << std::endl;
    }

    if(0){
    // 打印 allocationInfo
    std::cout << "    AllocationInfo:" << std::endl;
    std::cout << "    Priority: " << (int)allocationInfo.m_priority << std::endl;
    std::cout << "    Dynamic Scheduling: " << (allocationInfo.m_isDynamic ? "Yes" : "No") << std::endl;
    std::cout << "    HARQ Enabled: " << (allocationInfo.m_harqEnabled ? "Yes" : "No") << std::endl;
    std::cout << "    Transport Block Size: " << allocationInfo.m_tbSize << std::endl;
    }
    // std::cout << "    Allocated RLC PDUs: " << allocationInfo.m_allocatedRlcPdus.size() << " PDUs" << std::endl;
    // for (const auto& pdu : allocationInfo.m_allocatedRlcPdus)
    // {
    //     // 打印 RLC PDU 信息
    //     std::cout << "      RLC PDU Info: LCID=" << (int)pdu.lcid
    //               << ", Size=" << pdu.size << " bytes" << std::endl;
    // }

    // std::cout << "    Resource Reservation Interval (RRI): " << allocationInfo.m_rri.GetMilliSeconds()
    //           << " ms" << std::endl;

    // // 调用 DoNrSlAllocation
    // // 打印目标设备信息
    // std::cout << "Target Device Info (L2 ID): " << itDstInfo->second->GetDstL2Id() << std::endl;
    // std::cout << "  MCS: " << itDstInfo->second->GetDstMcs() << std::endl;

    // // 打印分配的资源
    // std::cout << "Allocated Resources (AllocList): " << std::endl;
    // for (const auto& resource : allocList)
    // {
    //     std::cout << "  Resource SFN: FrameNum: " << resource.sfn.GetFrame()
    //             << ", SubFrameNum: " << (int)resource.sfn.GetSubframe()
    //             << ", SlotNum: " << (int)resource.sfn.GetSlot() << std::endl;

    //     std::cout << "    Priority: " << resource.priority << std::endl;
    //     std::cout << "    MCS: " << resource.mcs << std::endl;
    //     std::cout << "    SubChannel: Start=" << (int)resource.slSubchannelStart
    //             << ", Length=" << (int)resource.slSubchannelLength << std::endl;
    //     std::cout << "    PSCCH: numSlPscchRbs=" << resource.numSlPscchRbs
    //             << ", slPscchSymStart=" << resource.slPscchSymStart
    //             << ", slPscchSymLength=" << resource.slPscchSymLength << std::endl;
    //     std::cout << "    PSSCH: slPsschSymStart=" << resource.slPsschSymStart
    ...
    // 打印分配的资源
    // std::cout << "  Allocation successful. Allocated resources:" << std::endl;
    if(0){
    for (const auto& alloc : allocList)
    {
        // std::cout << "    Allocated SFN: Frame=" << alloc.sfn.GetFrame()
        //           << ", SubFrame=" << (int)alloc.sfn.GetSubframe()
        //           << ", Slot=" << (int)alloc.sfn.GetSlot()
        //           << ", SubChStart=" << alloc.slPsschSubChStart
        //           << ", SubChLength=" << alloc.slPsschSubChLength
        //           << std::endl;
        std::cout << "\n Allocated SFN: Frame=" << alloc.sfn
            << ", SubChStart=" << alloc.slPsschSubChStart
            << ", SubChLength=" << alloc.slPsschSubChLength
            << std::endl;
    }
    ...
    // for (const auto& alloc : allocList)
    // {
    //     std::cout << "    Allocated SFN: Frame= " << alloc.sfn
    //         << ", SubChStart=" << alloc.slPsschSubChStart
    //         << ", SubChLength=" << alloc.slPsschSubChLength
    //         << std::endl;
    // }
    ...
    for (const auto& alloc : allocList)
    {
        std::cout << "====== allocList  SFN: " << alloc.sfn << "\n"
                  << "Destination Layer 2 ID: " << alloc.dstL2Id << "\n"
                  << "NDI: " << static_cast<int>(alloc.ndi) << "\n"
                  << "RV: " << static_cast<int>(alloc.rv) << "\n"
                  << "Priority: " << static_cast<int>(alloc.priority) << "\n"
                  << "MCS: " << alloc.mcs << "\n"
                  << "Num PSCCH RBs: " << alloc.numSlPscchRbs << "\n"
                  << "PSCCH Symbol Start: " << alloc.slPscchSymStart << "\n"
                  << "PSCCH Symbol Length: " << alloc.slPscchSymLength << "\n"
                  << "PSSCH Symbol Start: " << alloc.slPsschSymStart << "\n"
                  << "PSSCH Symbol Length: " << alloc.slPsschSymLength << "\n"
                  << "PSSCH SubChannel Start: " << alloc.slPsschSubChStart << "\n"
                  << "PSSCH SubChannel Length: " << alloc.slPsschSubChLength << "\n"
                  << "Max Num Per Reserve: " << alloc.maxNumPerReserve << "\n"
                  << "Has PSFCH: " << alloc.slHasPsfch << "\n"
                  << "Flag to indicate if the slots must carry SCI 1-A: " << alloc.txSci1A << "\n"
                  << "Slot Num Indication: " << static_cast<int>(alloc.slotNumInd) << "\n";
    }
    }
...
Time
NrSlUeMacSchedulerDynamicMcs::GetSpsGrantTimeout(const SfnSf& sfn,
...
void
NrSlUeMacSchedulerDynamicMcs::CreateSpsGrant(const SfnSf& sfn,
...
Time
NrSlUeMacSchedulerDynamicMcs::GetDynamicGrantTimeout(const SfnSf& sfn,
                                                   const std::set<SlGrantResource>& slotAllocList,
                                                   bool harqEnabled,
...
void
NrSlUeMacSchedulerDynamicMcs::CreateSinglePduGrant(const SfnSf& sfn,
                                                 const std::set<SlGrantResource>& slotAllocList,
                                                 const AllocationInfo& allocationInfo)
{
    NS_LOG_FUNCTION(this << sfn);
...
NrSlUeMacSchedulerDynamicMcs::CreateSpsGrantInfo(const std::set<SlGrantResource>& slotAllocList,
...
NrSlUeMacScheduler::GrantInfo
NrSlUeMacSchedulerDynamicMcs::CreateSinglePduGrantInfo(const std::set<SlGrantResource>& slotAllocList,
                                                     const AllocationInfo& allocationInfo) const
{
    NS_LOG_FUNCTION(this);
...
void
NrSlUeMacSchedulerDynamicMcs::CheckForGrantsToPublish(const SfnSf& sfn)
{
...
bool
NrSlUeMacSchedulerDynamicMcs::OverlappedResources(const SfnSf& firstSfn,
                                                uint16_t firstStart,
...
NrSlUeMacSchedulerDynamicMcs::FilterTxOpportunities(const SfnSf& sfn,
...
uint8_t
NrSlUeMacSchedulerDynamicMcs::GetTotalSubCh() const
{
    return GetMac()->GetTotalSubCh();
}

uint8_t
NrSlUeMacSchedulerDynamicMcs::GetSlMaxTxTransNumPssch() const
{
    return GetMac()->GetSlMaxTxTransNumPssch();
}

uint8_t
NrSlUeMacSchedulerDynamicMcs::GetRv(uint8_t txNumTb) const
{
...
int64_t
NrSlUeMacSchedulerDynamicMcs::AssignStreams(int64_t stream)
{
...
void
NrSlUeMacSchedulerDynamicMcs::DoDispose()
{
    NS_LOG_FUNCTION(this);
}

uint32_t
NrSlUeMacSchedulerDynamicMcs::CalculateTbSize(Ptr<const NrAmc> nrAmc,
                                            uint8_t dstMcs,
...
bool
NrSlUeMacSchedulerDynamicMcs::DoNrSlAllocation(
    const std::list<SlResourceInfo>& candResources,
    ...
    const AllocationInfo& allocationInfo)
{
    ...
    // std::cout << "GetMac()->GetPsfchPeriod() ===" << static_cast<int>(GetMac()->GetPsfchPeriod()) << std::endl;
    if (allocationInfo.m_harqEnabled && (GetMac()->GetPsfchPeriod() == 0))
    {
        ...
        // std::cout << "1candResources: " << candResources.size() << std::endl;
        selectedTxOpps = SelectResourcesForBlindRetransmissions(candResources);
        // std::cout << "1selectedTxOpps: " << selectedTxOpps.size() << std::endl;
        // selectedTxOpps = SelectResourcesForBlindRetransmissions2D(candResources);
    }
    else
    {
        std::cout << "2candResources: " << candResources.size() << std::endl;
        selectedTxOpps = SelectResourcesWithConstraint(candResources, allocationInfo.m_harqEnabled);
        std::cout << "2selectedTxOpps: " << selectedTxOpps.size() << std::endl;
    }
    ...
    {
        // std::cout << "=========itTxOpps_iter=========" << itTxOpps_iter << std::endl;
        itTxOpps_iter++;
        ...
        // std::cout << "====== allocList  SFN: " << slotAlloc.sfn << "\n"
        //     << "Destination Layer 2 ID: " << slotAlloc.dstL2Id << "\n"
        //     << "NDI: " << static_cast<int>(slotAlloc.ndi) << "\n"
        //     << "RV: " << static_cast<int>(slotAlloc.rv) << "\n"
        //     << "Priority: " << static_cast<int>(slotAlloc.priority) << "\n"
        //     << "MCS: " << slotAlloc.mcs << "\n"
        //     << "Num PSCCH RBs: " << slotAlloc.numSlPscchRbs << "\n"
        //     << "PSCCH Symbol Start: " << slotAlloc.slPscchSymStart << "\n"
        //     << "PSCCH Symbol Length: " << slotAlloc.slPscchSymLength << "\n"
        //     << "PSSCH Symbol Start: " << slotAlloc.slPsschSymStart << "\n"
        //     << "PSSCH Symbol Length: " << slotAlloc.slPsschSymLength << "\n"
        //     << "PSSCH SubChannel Start: " << slotAlloc.slPsschSubChStart << "\n"
        //     << "PSSCH SubChannel Length: " << slotAlloc.slPsschSubChLength << "\n"
        //     << "Max Num Per Reserve: " << slotAlloc.maxNumPerReserve << "\n"
        //     << "Has PSFCH: " << slotAlloc.slHasPsfch << "\n"
        //     << "Flag to indicate if the slots must carry SCI 1-A: " << slotAlloc.txSci1A << "\n"
        //     << "Slot Num Indication: " << static_cast<int>(slotAlloc.slotNumInd) << "\n";
...
bool
NrSlUeMacSchedulerDynamicMcs::OverlappedSlots(const std::list<SlResourceInfo>& resources,
...
bool
NrSlUeMacSchedulerDynamicMcs::IsResourceOverlapped(const std::list<SlResourceInfo>& resources,
                                            const SlResourceInfo& candidate) const
{
    for (const auto& it : resources)
    {
        // 检查时间维度是否重叠
        if (it.sfn == candidate.sfn &&
            it.sfn.GetSlot() == candidate.sfn.GetSlot())
        {
            // 检查频率维度是否重叠
            int candidateStart = candidate.slSubchannelStart;
            int candidateEnd = candidateStart + candidate.slSubchannelLength;
            int existingStart = it.slSubchannelStart;
            int existingEnd = existingStart + it.slSubchannelLength;
...
std::list<SlResourceInfo>
NrSlUeMacSchedulerDynamicMcs::SelectResourcesForBlindRetransmissions(std::list<SlResourceInfo> txOpps)
{
    NS_LOG_FUNCTION(this << txOpps.size());

    uint8_t totalTx = GetSlMaxTxTransNumPssch();
    // std::cout << "totalTx ===" << static_cast<int>(totalTx) << std::endl;
...
std::list<SlResourceInfo> 
NrSlUeMacSchedulerDynamicMcs::SelectResourcesForBlindRetransmissions2D(std::list<SlResourceInfo> txOpps)
{
    NS_LOG_FUNCTION(this << txOpps.size());
    ...
    if (txOpps.empty()) {
        NS_LOG_DEBUG("No candidate resources available");
        return newTxOpps;
    }

    // 首先确定时间和频率维度的范围
    uint32_t maxSlot = 0;
    uint32_t maxSubchannel = 0;
    for (const auto& res : txOpps) {
        maxSlot = std::max(maxSlot, static_cast<uint32_t>(res.sfn.Normalize()));
        maxSubchannel = std::max(maxSubchannel,
                                static_cast<uint32_t>(res.slSubchannelStart + res.slSubchannelLength));
    }

    // 创建2D资源矩阵 (slot x subchannel)
    std::vector<std::vector<bool>> resourceGrid(maxSlot + 1, 
                                              std::vector<bool>(maxSubchannel, false));

    // 如果候选资源数量大于所需传输次数
    if (txOpps.size() > totalTx) {
        while (newTxOpps.size() < totalTx && !txOpps.empty()) {
            // 随机选择一个索引
            uint32_t randomIndex = m_grantSelectionUniformVariable->GetInteger(0, txOpps.size() - 1);
            auto txOppsIt = txOpps.begin();
            std::advance(txOppsIt, randomIndex);

            // 检查该时隙是否已被使用
            bool slotAvailable = true;
            uint32_t slot = static_cast<uint32_t>(txOppsIt->sfn.Normalize());
            
            // 检查是否与已选资源重叠
            for (const auto& selectedRes : newTxOpps) {
                if (selectedRes.sfn == txOppsIt->sfn) {
                    slotAvailable = false;
                    break;
                }
            }
            ...
            if (slotAvailable) {
                newTxOpps.push_back(*txOppsIt);
                NS_LOG_DEBUG("Selected slot " << slot 
                            << " subchannels [" << txOppsIt->slSubchannelStart 
                            << "," << txOppsIt->slSubchannelStart + txOppsIt->slSubchannelLength - 1 
                            << "]");
            }
        ...
        // 如果候选资源数量不大于所需传输次数，尝试使用所有可用资源
        for (auto txOppsIt = txOpps.begin(); txOppsIt != txOpps.end();) {
            bool slotAvailable = true;
            ...
            if (slotAvailable) {
                newTxOpps.push_back(*txOppsIt);
                NS_LOG_DEBUG("Selected slot " << static_cast<uint32_t>(txOppsIt->sfn.Normalize()));
            }
    ...
    // 确保至少选择了一个资源
    NS_ASSERT_MSG(!newTxOpps.empty(), "Scheduler should select at least 1 slot from txOpps");
...
//     // 标记已占用的资源
//     for (const auto& res : txOpps) {
//         uint32_t slot = res.sfn.Normalize();
//         for (uint32_t subch = res.slSubchannelStart; 
//              subch < res.slSubchannelStart + res.slSubchannelLength; 
//              subch++) {
//             resourceGrid[slot][subch] = true;
//         }
//     }
...
//             uint32_t slot = txOppsIt->sfn.Normalize();
            
//             // 检查该时隙的所有子信道是否已被占用
//             for (uint32_t subch = txOppsIt->slSubchannelStart; 
//                  subch < txOppsIt->slSubchannelStart + txOppsIt->slSubchannelLength; 
//                  subch++) {
//                 if (resourceGrid[slot][subch]) {
//                     hasOverlap = true;
//                     break;
//                 }
//             }
...
//         NS_LOG_DEBUG("Resource allocation grid:");
//         for (uint32_t slot = 0; slot < resourceGrid.size(); slot++) {
//             std::string row = "Slot " + std::to_string(slot) + ": ";
//             for (uint32_t subch = 0; subch < resourceGrid[slot].size(); subch++) {
//                 row += resourceGrid[slot][subch] ? "1 " : "0 ";
//             }
//             NS_LOG_DEBUG(row);
//         }
//     }
...
//     int numFrequencyBlocks = GetNumFrequencyBlocks();
//     std::vector<std::vector<bool>> resourceMatrix(numSlots, std::vector<bool>(numFrequencyBlocks, false));

//     // 填充资源矩阵
//     for (const auto& resource : txOpps)
//     {
//         int timeIndex = CalculateTimeIndex(resource.sfn, resource.subFrameNum, resource.slotNum);
//         int freqStart = resource.slSubchannelStart;
//         int freqEnd = freqStart + resource.slSubchannelLength;

//         for (int freqIndex = freqStart; freqIndex < freqEnd; ++freqIndex)
//         {
//             resourceMatrix[timeIndex][freqIndex] = true;
//         }
//     }
...
//         {
//             int randomTimeIndex = m_grantSelectionUniformVariable->GetInteger(0, numSlots - 1);
//             int randomFreqIndex = m_grantSelectionUniformVariable->GetInteger(0, numFrequencyBlocks - 1);

//             if (resourceMatrix[randomTimeIndex][randomFreqIndex] && 
//                 !IsResourceOverlapped(newTxOpps, randomTimeIndex, randomFreqIndex))
//             {
//                 SlResourceInfo selectedResource;
//                 selectedResource.sfn = randomTimeIndex;
//                 selectedResource.slSubchannelStart = randomFreqIndex;
//                 selectedResource.slSubchannelLength = 1;

//                 newTxOpps.emplace_back(selectedResource);
//                 resourceMatrix[randomTimeIndex][randomFreqIndex] = false;
//             }
//         }
//     }
//     else
//     {
//         for (int timeIndex = 0; timeIndex < numSlots; ++timeIndex)
//         {
//             for (int freqIndex = 0; freqIndex < numFrequencyBlocks; ++freqIndex)
//             {
//                 if (resourceMatrix[timeIndex][freqIndex] && 
//                     !IsResourceOverlapped(newTxOpps, timeIndex, freqIndex))
//                 {
//                     SlResourceInfo selectedResource;
//                     selectedResource.sfn = timeIndex;
//                     selectedResource.slSubchannelStart = freqIndex;
//                     selectedResource.slSubchannelLength = 1;

//                     newTxOpps.emplace_back(selectedResource);
//                     resourceMatrix[timeIndex][freqIndex] = false;
//                 }
//             }
//         }
//     }
...
std::list<SlResourceInfo>
NrSlUeMacSchedulerDynamicMcs::SelectResourcesWithConstraint(std::list<SlResourceInfo> txOpps,
                                                          bool harqEnabled)
{
...
bool
NrSlUeMacSchedulerDynamicMcs::IsMinTimeGapSatisfied(const SfnSf& first,
                                                  const SfnSf& second,
...
bool
NrSlUeMacSchedulerDynamicMcs::IsCandidateResourceEligible(const std::list<SlResourceInfo>& txOpps,
...
Ptr<NrSlUeMacHarq>
NrSlUeMacSchedulerDynamicMcs::GetMacHarq(void) const
{
    if (!m_nrSlUeMacHarq)
    {
        PointerValue val;
