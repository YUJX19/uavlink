...
struct RxPacketTraceParams
{
    RxPacketTraceParams(TransportBlockInfo tbInfo,
                        ...
                        uint8_t cqi)
        : 
          m_sinrPerceived(tbInfo.m_sinrPerceived),
          m_rbBitmap(tbInfo.m_expected.m_rbBitmap),
          m_cellId(cellId),
          m_rnti(rnti),
          ...
          m_cqi(cqi){
            ...
            {
                std::cout << "tbInfo.m_expected.m_rbBitmap is empty" << std::endl;
            }
            else
            {
                std::cout << "tbInfo.m_expected.m_rbBitmap size: " << tbInfo.m_expected.m_rbBitmap.size() << std::endl;
                for (const auto& rb : tbInfo.m_expected.m_rbBitmap)
                {
                    std::cout << rb << " ";
                }
                std::cout << std::endl;
